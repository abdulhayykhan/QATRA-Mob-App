...
class _AdminVerificationQueueScreenState extends ConsumerState<AdminVerificationQueueScreen> {
  ...
  Widget build(BuildContext context) {
    final currentSlip = _pendingSlips[_selectedQueueIndex];
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  Text(
                    'Active Verification (${_pendingSlips.length} in Queue)',
                    style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15),
                  ),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(color: const Color(0xFFFEF2F2), borderRadius: BorderRadius.circular(8)),
                    child: const Text('Target SLA: < 3 mins', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppColors.primaryRed)),
                  ),
                ],
              ),
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: List.generate(_pendingSlips.length, (index) {
                    final isSelected = _selectedQueueIndex == index;
                    final item = _pendingSlips[index];
                    return Padding(
                      padding: const EdgeInsets.only(right: 8),
                      child: ChoiceChip(
                        label: Text('${item['id']} • ${item['bloodGroup']}'),
                        selected: isSelected,
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                            decoration: BoxDecoration(
                              color: currentSlip['flagged'] == true ? const Color(0xFFFFF3E0) : const Color(0xFFE8F5E9),
                              borderRadius: BorderRadius.circular(8),
                            ),
                            child: Text(
                              ...
                              style: TextStyle(
                                fontSize: 10,
                                fontWeight: FontWeight.bold,
                                color: currentSlip['flagged'] == true ? const Color(0xFFE65100) : const Color(0xFF2E7D32),
                              ),
                            ),
                          ),
                        ],
                      ...
                      const Divider(height: 16),
                      _buildOcrRow('Blood Group', currentSlip['bloodGroup'], Icons.bloodtype),
                      const Divider(height: 16),
                ...
                children: [
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text('Request ${currentSlip['id']} approved & broadcasted to nearby donors!'),
                            backgroundColor: const Color(0xFF2E7D32),
                          ),
                        );
                        setState(() {
                          if (_pendingSlips.length > 1) {
                            _pendingSlips.removeAt(_selectedQueueIndex);
                  ...
                  Expanded(
                    ...
                    child: OutlinedButton(
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Request ${currentSlip['id']} rejected.')),
                        );
                      },
